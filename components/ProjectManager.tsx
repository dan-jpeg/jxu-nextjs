...
const ProjectManager: React.FC<ProjectManagerProps> = ({
    ...
    const handleReorder = async (projectId: string, direction: "up" | "down") => {
        ...
        // Enforce per-category ordering (matches archive display).
        if (project1.category !== project2.category) return;

        try {
            // Recompute sequential order within this category to avoid duplicate
            // orders causing "swap" to appear like a no-op.
            const category = project1.category;
            const categoryIndices = projects
                .map((p, idx) => (p.category === category ? idx : -1))
                .filter((idx) => idx !== -1);

            const categoryProjects = categoryIndices.map((idx) => projects[idx]);
            const idxInCategory = categoryIndices.indexOf(currentIndex);
            const newIdxInCategory = idxInCategory + (direction === "up" ? -1 : 1);
            if (newIdxInCategory < 0 || newIdxInCategory >= categoryProjects.length) return;

            const reordered = [...categoryProjects];
            [reordered[idxInCategory], reordered[newIdxInCategory]] = [
                reordered[newIdxInCategory],
                reordered[idxInCategory],
            ];

            const updates = reordered
                .map((p, idx) => ({ id: p.id, order: idx }))
                .filter((u) => projects.find((p) => p.id === u.id)?.order !== u.order);
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <button
                                            ...
                                            disabled={index === 0 || projects[index - 1]?.category !== project.category}
                                        ...
                                        <button
                                            ...
                                            disabled={index === projects.length - 1 || projects[index + 1]?.category !== project.category}
                                            className="text-gray-400 hover:text-gray-600 disabled:opacity-30 disabled:cursor-not-allowed"
                                            title="Move down"
                                        >
                                            ▼
